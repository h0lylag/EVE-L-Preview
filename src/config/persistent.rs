...
impl PersistentState {
    /// Access character positions (single source of truth)
    pub fn character_positions(&self) -> &HashMap<String, CharacterSettings> {
        &self.profile.character_positions
    }
    
    /// Mutable access to character positions
    pub fn character_positions_mut(&mut self) -> &mut HashMap<String, CharacterSettings> {
        &mut self.profile.character_positions
    }
...
impl PersistentState {
    ...
    pub fn save(&self) -> Result<()> {
        ...
        // Load existing config to preserve GUI-managed settings
        ...
        // Find the selected profile and update ONLY character positions
        ...
        // Replace character positions with our current state (single source of truth)
        profile_config.profiles[profile_idx].character_positions = self.profile.character_positions.clone();
        
        // Save using OverwriteCharacterPositions (daemon owns this data)
    ...
    pub fn update_position(&mut self, character_name: &str, x: i16, y: i16, width: u16, height: u16) -> Result<()> {
        if !character_name.is_empty() {
            ...
            self.profile.character_positions.insert(character_name.to_string(), settings);
    ...
    pub fn handle_character_change(
        ...
        // Save old character's position and current dimensions to single source of truth
        if !old_name.is_empty() {
            ...
            self.profile.character_positions.insert(old_name.to_string(), settings);
        }
        ...
        if !new_name.is_empty() {
            if let Some(settings) = self.profile.character_positions.get(new_name) {
...
mod tests {
    ...
    fn test_update_position_with_character_name() {
        ...
        let settings = state.profile.character_positions.get("TestChar").unwrap();
    ...
    fn test_update_position_empty_name_ignored() {
        ...
        assert!(state.profile.character_positions.is_empty());
    }
    ...
    fn test_handle_character_change_both_names() {
        ...
        state.profile.character_positions.insert("NewChar".to_string(), CharacterSettings::new(500, 600, 240, 135));
        ...
        let old_settings = state.profile.character_positions.get("OldChar").unwrap();
        ...
        let new_settings = state.profile.character_positions.get("NewChar").unwrap();
    ...
    fn test_handle_character_change_logout() {
        ...
        // Should save old position AND dimensions (even if disk save fails)
        let settings = state.profile.character_positions.get("LoggingOut").unwrap();
